	...
	public partial class Form1 : Form
	{
		...
		private void make_fract(string fileName, int level)
		{
			...
			for (int i = 0; i < actions.Length; i++)
			{
				...
				{
					stack.Push(new State(currentState.angle, 
                        new My_point(currentState.prevPoint.X, currentState.prevPoint.Y), 
                        new My_point(currentState.nextPoint.X, currentState.nextPoint.Y)));
				}
				else if (actions[i] == ']')
				{
        ...
        class DiamondSquare
        {
            ...
            public void Iterate()
            {
                ...
                for (int i = 0; i < gens; ++i)
                {
                    ...
                    while (cur.Next != null)
                    {
                        float _length = (float)Math.Sqrt(Math.Pow((double)cur.Value.Y - (double)cur.Next.Value.Y, 2) + Math.Pow((double)cur.Value.X - (double)cur.Next.Value.X, 2));
                        float h = Math.Abs(cur.Value.Y - cur.Next.Value.Y) / 2 + cur.Value.Y;
                        h += (float)r.Next(-10, 10);
